...
void remapk4a(const k4a_image_t src, const k4a_image_t lut, k4a_image_t dst, interpolation_t type)
{
    ...
    if (k4a_image_get_format(src) == K4A_IMAGE_FORMAT_DEPTH16)
    {
    ...
    else if (k4a_image_get_format(src) == K4A_IMAGE_FORMAT_COLOR_BGRA32)
    {
        // note: we get k4a color images in bgra format, so 4 channels each 8 bit
        coordinate_t *lut_data = (coordinate_t *)(void *)k4a_image_get_buffer(lut);
        ...
        // uint8_t *dst_data_8 =  (uint8_t *)dst_data; // TODO for bilinear: this gives different adress than dst_data but should be the same to work
        for (int i = 0; i < dst_width * dst_height; i++)
        {
            if (lut_data[i].x != INVALID && lut_data[i].y != INVALID)
            {
                ...
                // TODO bilinear is buggy because of crazy pointer cast stuff
                // else if (type == INTERPOLATION_BILINEAR)
                // {
                ...
                //                                 &src_data[(lut_data[i].y + 1) * src_width + lut_data[i].x + 1]};
                //     // //blue
                //     const uint8_t *neighborsB[4] = {(uint8_t *)neighbors[0],
                //                                     (uint8_t *)neighbors[1],
                //                                     (uint8_t *)neighbors[2],
                //                                     (uint8_t *)neighbors[3]};
                //     //green
                //     const uint8_t *neighborsG[4] = {(uint8_t *)neighbors[0] + 1,
                //                                     (uint8_t *)neighbors[1] + 1,
                //                                     (uint8_t *)neighbors[2] + 1,
                //                                     (uint8_t *)neighbors[3] + 1};
                //     //red
                //     const uint8_t *neighborsR[4] = {(uint8_t *)neighbors[0] + 2,
                //                                     (uint8_t *)neighbors[1] + 2,
                //                                     (uint8_t *)neighbors[2] + 2,
                //                                     (uint8_t *)neighbors[3] + 2};
                //
                //     *dst_data_8++ = (uint8_t)(*neighborsB[0] * lut_data[i].weight[0] + *neighborsB[1] * lut_data[i].weight[1] +
                //                               *neighborsB[2] * lut_data[i].weight[2] + *neighborsB[3] * lut_data[i].weight[3]);
                //     *dst_data_8++ = (uint8_t)(*neighborsG[0] * lut_data[i].weight[0] + *neighborsG[1] * lut_data[i].weight[1] +
                //                               *neighborsG[2] * lut_data[i].weight[2] + *neighborsG[3] * lut_data[i].weight[3]);
                //     *dst_data_8++ = (uint8_t)(*neighborsR[0] * lut_data[i].weight[0] + *neighborsR[1] * lut_data[i].weight[1] +
                //                               *neighborsR[2] * lut_data[i].weight[2] + *neighborsR[3] * lut_data[i].weight[3]);
                //     dst_data_8++; // skip alpha channel
                // }
                else
                {
                    printf("Unexpected interpolation type!\n");
    ...
    {
        cout << "WARNING: Unexpected src image type in remapk4a!" << endl;
        exit(-1);
    }
}
